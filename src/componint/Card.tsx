...
const Card = ({ product, open }: iprops) => {
    ...
    <div className="  flex flex-col   border border-gray-200 rounded-md mx-2 my-2   p-2">
      ...
      <div className=" p-3">
        ...
        <div className="flex space-x-2 my-2 ">
          <span className={`bg-['${colors[0]}'] rounded-full w-4  h-4`} />
          <span className="bg-blue-300 rounded-full w-4  h-4" />
          <span className="bg-amber-900 rounded-full w-4 h-4" />
        </div>
        ...
        <div className="btn flex flex-row h-11 mt-2 space-x-2">
          <Button classname={" bg-indigo-700"} width={"w-full"} onClick={open}>
            EDITE
          </Button>
